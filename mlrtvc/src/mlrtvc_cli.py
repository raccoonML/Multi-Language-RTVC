...
if __name__ == "__main__":
    ...
    enc_model_fpath = (
        args.saved_models_dir.joinpath(args.language_code)
        .joinpath(args.model_name)
        .joinpath("encoder/encoder.pt")
    )
    syn_model_fpath = (
        args.saved_models_dir.joinpath(args.language_code)
        .joinpath(args.model_name)
        .joinpath("synthesizer/synthesizer.pt")
    )
    voc_model_fpath = (
        args.saved_models_dir.joinpath(args.language_code)
        .joinpath(args.model_name)
        .joinpath("vocoder/vocoder.pt")
    )
    ...
    while True:
        try:
            ...
            sf.write(
                filename, generated_wav.astype(np.float32), synthesizer.sample_rate
            )
            num_generated += 1
            print("\nSaved output as %s\n\n" % filename)
