...
from core.encoder.inference import plot_embedding_as_heatmap
from core.toolbox.utterance import Utterance
...
class UI(QDialog):
    ...
    def populate_models(self, saved_models_dir: Path, language_code: str):
        ...
        self.repopulate_box(
            self.encoder_box, [(f.parent.parent.stem, f) for f in encoder_fpaths]
        )
        ...
        if len(synthesizer_fpaths) == 0:
            raise Exception(
                "No synthesizer models found in %s" % synthesizer_models_dir
            )
        self.repopulate_box(
            self.synthesizer_box,
            [(f.parent.parent.stem, f) for f in synthesizer_fpaths],
        )

        # Vocoder
        vocoder_fpaths = list(model_dir.glob("**/vocoder/vocoder.pt"))
        vocoder_items = [(f.parent.parent.stem, f) for f in vocoder_fpaths] + [
            ("Griffin-Lim", None)
        ]
